...
public protocol MediaViewerControllerDelegate: class {
    ...
    func mediaViewerController(_ controller: MediaViewerController, didTouch image: ImageModel, at index: Int)
}
...
open class MediaViewerController: UIViewController {
    ...
    open weak var delegate: MediaViewerControllerDelegate?
    ...
    fileprivate let initialImages: [ImageModel]
    fileprivate let initialPage: Int

    // MARK: - Initializers

    public init(images: [ImageModel] = [], startIndex index: Int = 0) {
    ...
    open override func viewDidAppear(_ animated: Bool) {
        ...
        if MediaViewerConfig.hideStatusBar {
    ...
    func configurePages(_ images: [ImageModel]) {
        pageViews.forEach {
...
//    footerView.expand(false)
    ...
    open func configureLayout(_ size: CGSize = UIScreen.main.bounds.size) {
        ...
        [headerView, footerView].forEach {
            ($0 as AnyObject).configureLayout()
        }
...
extension MediaViewerController: PageViewDelegate {
    ...
    func pageViewDidTouch(_ pageView: PageView) {
        guard !pageView.hasZoomed else {
            return
        }

        delegate?.mediaViewerController(self, didTouch: images[currentPage], at: currentPage)
